    ...
    resolveCombatAbility: function (requestUid, targetKeys, overrides) {
      ...
      return ensureReady().then(function(user){
        ...
        return readRoom(session.code).then(function(room){
          ...
          if(actorIndex<0||!targetIndexes.length||targetIndexes.some(function(index){return index<0;}))throw roomError('Участник способности не найден.','combat-participant-missing');
          ...
          var range=Math.max(0,Number(effect.rangeCells)||0);targetIndexes.forEach(function(targetIndex){var distance=combatEntryDistance(room,actor,order[targetIndex]);if(range&&distance!=null&&distance>range)throw roomError((order[targetIndex].name||'Цель')+' слишком далеко: '+distance+' кл., дальность способности '+range+' кл.','combat-target-range');});
          var mode=effect.resolutionMode,results=[],damageType=String(effect.damageType||'');
          targetIndexes.forEach(function(targetIndex){
            var target=combatEntryWithRoomStatuses(room,Object.assign({},order[targetIndex])),natural=null,rolls=[],modifier=0,total=null,success=true,dc=null;
            ...
            var applyStatuses=mode==='save'?!success:success;if(applyStatuses&&effect.statuses.length){target.statuses=Array.isArray(target.statuses)?target.statuses.slice():[];effect.statuses.forEach(function(key){if(combatStatusKeys(target).indexOf(key)<0)target.statuses.push(key);});}
          ...
          if(cost!=='free')economy[cost]=Math.max(0,Number(economy[cost]||0)-1);if(restrictions.slowed&&(cost==='long'||cost==='short')){economy.long=0;economy.short=0;}order[actorIndex].economy=economy;
