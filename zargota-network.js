(function (w) {
  ...
  function terminalGameplayError(error) {
    ...
    return [
      ...
      'spell-invalid','spell-not-owned','spell-not-learned','spell-not-prepared'
  ...
  function applyAbilityUsageDomainOperation(usage, resourceKey, operation) {
    ...
    return{usage:usage,state:nextState,resourceKey:resourceKey,previousUsed:previousUsed,used:used,max:max,changed:!!resourceKey&&(scopeChanged||used!==previousUsed)};
  }
  ...
  function characterSnapshot(character) {
    ...
    spellRefs.forEach(function (id) {
      ...
      if (resourceId) {
        ...
        abilityUsage['spell-' + resourceId] = { used:Math.max(0, Number(state.used) || 0), max:Math.max(0, Number(state.max) || 0), scopeKey:String(state.scopeKey||'').slice(0,120), scopeKind:String(state.scopeKind||'').slice(0,30) };
      }
      ...
      return ensureReady().then(function (user) {
        ...
        return readRoom(session.code).then(function (room) {
          ...
          if (details && actionKind === 'ability') {
            ...
            var scopedUsed=usage&&(!battleScope||String(usage.scopeKey||'')===battleScope)?Number(usage.used)||0:0;
        ...
        if (error && ['room-required','room-not-found','player-missing','request-pending','ability-exhausted','combat-zero-hp','combat-missing','combat-not-owner','combat-not-turn','combat-status-blocked','combat-action-spent','spell-learning-invalid','spell-already-learned','spell-invalid','spell-not-owned','spell-not-learned','spell-not-prepared','hero-incapacitated'].indexOf(error.code) >= 0) throw error;
        throw friendlyFirebaseError(error);
      });
    },
      ...
      return ensureReady().then(function (user) {
        ...
        return readRoom(session.code).then(function (room) {
          ...
          if(expiredSummons.indexOf(ending)<0)queueCombatEntryState(room,updates,ending,true,{
            ...
            writeScene:masterAdvance&&!expiredSummons.length
          });
        ...
        if (error && ['session-missing','master-only','room-not-found','combat-missing','turn-owner-only','fate-decision-required','death-save-required','combat-no-active-participants'].indexOf(error.code) >= 0) throw error;
        throw friendlyFirebaseError(error);
      });
    },
    ...
    resolveCombatAttack: function (targetKey, options, participantKey) {
      ...
      return ensureReady().then(function (user) {
        ...
        return readRoom(session.code).then(function (room) {
          ...
          var profiles=combatWeaponProfiles(attacker),weaponId=String(options.weaponId||''),weapon=profiles.filter(function(item){return item&&String(item.id)===weaponId;})[0]||profiles[0];
          ...
          var distance=combatEntryDistance(room,attacker,target),rangeCells=combatRangeCells(weapon.range||'1 клетка'),rangeOverride=options.rangeOverride===true;
          ...
          var attackTotal=natural+statBonus+masteryBonus+attackerModifiers.attackMod+(Number(attackerOverride.attackMod)||0),targetAc=Math.max(0,(Number(target.ac)||10)+targetModifiers.acMod+(Number(targetOverride.acMod)||0)),critical=natural===20,failed=natural===1,hit=!failed&&(critical||attackTotal>=targetAc);
          ...
          updates['combat/order']=order;updates['combat/updatedAt']=stamp;updates['combat/appliedActionOperationIds']=combat.appliedActionOperationIds;if(preparedAttack)updates['combat/pendingPreparedAction']=null;if(consumedAttackStatuses.length&&attacker.uid){updates['members/'+attacker.uid+'/character/statuses']=attacker.statuses||[];updates['members/'+attacker.uid+'/character/statusEffects']=attacker.statusEffects||[];}
    ...
    resolveCombatDamage: function (targetKey, options, participantKey) {
      ...
      return ensureReady().then(function(user){
        ...
        return readRoom(session.code).then(function(room){
          ...
          var profiles=combatWeaponProfiles(attacker),weaponId=String(options.weaponId||''),weapon=profiles.filter(function(item){return item&&String(item.id)===weaponId;})[0]||profiles[0];
          var damageDistance=combatEntryDistance(room,attacker,target),damageRangeCells=combatRangeCells(weapon.range||'1 клетка');
          var statKey=['str','dex','int','cha','per','con'].indexOf(options.statKey)>=0?options.statKey:(weapon.stat||'str');
    ...
    resolveCombatAbility: function (requestUid, targetKeys, overrides) {
      ...
      return ensureReady().then(function(user){
        ...
        return readRoom(session.code).then(function(room){
          ...
          var requestStatus=String(request&&request.status||''),stagedAutomation=!!(ability&&String(ability.automationKey||'')==='finger-heat-v1'&&String(ability.playbackMode||'')!=='instant');
          if(!request||!ability||(stagedAutomation?['ability-attack-result','ability-damage-result'].indexOf(requestStatus)<0:requestStatus!=='pending'))throw roomError(stagedAutomation?'Сначала выполните назначенные броски заклинания.':'Заявка способности уже обработана.','request-missing');
          ...
          var approvedResults={};
          if(String(effect.automationKey||'')==='finger-heat-v1'&&Array.isArray(overrides.approvedResults))overrides.approvedResults.slice(0,30).forEach(function(raw){
            if(!raw||typeof raw!=='object')return;var key=String(raw.key||'').slice(0,160);if(!key)return;
            var approvedStatuses=sanitizeAbilityStatusKeys(raw.statuses,6),prescribedStatuses=combatAutomationPrescribedStatusKeys(effect);
            approvedResults[key]={key:key,roll:Math.max(1,Math.min(20,Math.floor(Number(raw.roll)||1))),rolls:(Array.isArray(raw.rolls)?raw.rolls:[raw.roll]).map(function(value){return Math.max(1,Math.min(20,Math.floor(Number(value)||1)));}).slice(0,2),rollMode:['normal','advantage','disadvantage'].indexOf(raw.rollMode)>=0?raw.rollMode:'normal',modifier:Math.max(-99,Math.min(99,Math.floor(Number(raw.modifier)||0))),total:Math.max(-99,Math.min(199,Math.floor(Number(raw.total)||0))),dc:Math.max(0,Math.min(199,Math.floor(Number(raw.dc)||0))),success:!!raw.success,damage:Math.max(0,Math.min(999,Math.floor(Number(raw.damage)||0))),damageRolls:(Array.isArray(raw.damageRolls)?raw.damageRolls:[]).map(function(value){return Math.max(1,Math.min(100,Math.floor(Number(value)||1)));}).slice(0,24),statuses:raw.success?sanitizeAbilityStatusKeys(prescribedStatuses.concat(approvedStatuses),12):[]};
          });
          if(stagedAutomation){
          ...
          var resourceKey=String(effect.resourceKey||'').replace(/[^a-zA-Z0-9_-]/g,'').slice(0,100),resourceMax=masterControlled?0:Math.max(0,Math.min(99,Number(effect.resourceMax)||0)),resourceDelta=effect.effectKind==='summon'?Math.max(1,Math.min(3,Number(effect.summonChargeCost)||1)):1,battleStartedAt=Number(combat.battleStartedAt||combat.startedAt||0),battleScopeKey=effect.resourceScopeKind==='battle'&&battleStartedAt>0?'battle-'+battleStartedAt:'',resourceMutation=resourceKey&&resourceMax?applyAbilityUsageDomainOperation(member.character&&member.character.abilityUsage,resourceKey,{delta:resourceDelta,max:resourceMax,minimumUsed:effect.resourceUsed,preserveExistingMax:false,scopeKey:battleScopeKey,scopeKind:effect.resourceScopeKind,resetWhenScopeChanges:effect.resourceScopeKind==='battle'&&!!battleScopeKey}):null;if(resourceMax&&(resourceKey?(!resourceMutation||resourceMutation.used-resourceMutation.previousUsed<resourceDelta):Math.max(0,Number(effect.resourceUsed)||0)+resourceDelta>resourceMax))throw roomError('Для этого розыгрыша не хватает зарядов способности.','ability-exhausted',null,'Для цього розіграшу бракує зарядів здібності.');
          ...
          var castStamp=now(),effectSource='ability-'+abilityOwnerKey+'-'+castStamp,concentrationTouched=[];
          ...
          var mode=effect.resolutionMode,results=[],damageType=String(effect.damageType||''),actorModifiers=combatStatusModifiers(actor),sharedAreaDamage=effect.areaMode!=='manual'&&effect.damageFormula&&!Object.keys(approvedResults).length?rollFormula(effect.damageFormula,false):null,sharedSuccessDamage=effect.successDamageFormula&&!Object.keys(approvedResults).length?rollFormula(effect.successDamageFormula,false):null,sharedFailDamage=effect.failDamageFormula&&!Object.keys(approvedResults).length?rollFormula(effect.failDamageFormula,false):null,sourceDamageRoll=effect.sourceDamageFormula?rollFormula(effect.sourceDamageFormula,false):null,sourceBeforeHp=Math.max(0,Number(actor.hp==null?actor.hpMax:actor.hp)||0),sourceRawDamage=sourceDamageRoll?Math.max(0,Number(sourceDamageRoll.total)||0)+combatStat(actor,effect.sourceDamageStat||'int'):0,sourceDamage=sourceDamageRoll?Math.min(sourceBeforeHp,sourceRawDamage):0,martyr=!!(sourceDamageRoll&&sourceBeforeHp<=Math.max(0,Number(effect.martyrHpThreshold)||0)),severeConcentrationSources=[];
        ...
        if(error&&['master-only','room-not-found','request-missing','roll-required','combat-missing','combat-participant-missing','combat-target-invalid','combat-area-anchor','combat-not-turn','combat-zero-hp','combat-status-blocked','combat-action-spent','combat-target-range','combat-ability-preview-invalid','ability-exhausted','summon-limit','summon-requirements','scene-token-limit'].indexOf(error.code)>=0)throw error;
        throw friendlyFirebaseError(error);
      });
    },
      ...
      return ensureReady().then(function(user){
        ...
        return firebase.runTransaction(roomRef(session.code),function(room){
          ...
          if(accepted){
            ...
            var execution={id:'reaction-execution-'+requestId,requestId:requestId,sourceKind:'reaction-request',participantKey:String(entry.key||''),participantUid:String(requestUid),actionKind:String(request.actionKind||'custom'),actionText:String(request.actionText||'Реакция'),targetKey:String(request.targetKey||''),targetName:String(request.targetName||''),weaponId:String(request.weaponId||''),weaponName:String(request.weaponName||''),text:String(request.actionText||'Реакция')+(request.detail?' · '+String(request.detail):''),stage:'ready',createdAt:stamp};
            if(attack)combat.pendingPreparedAction=execution;else combat.pendingReactionAction=execution;
          }
          combat.updatedAt=stamp;room.combatEvent={id:'combat-reaction-answer-'+requestId,kind:'combat-reaction-answer',name:String(entry.name||member.name||'Игрок'),portrait:String(entry.portrait||''),text:accepted?'Принимает предложенную реакцию «'+String(request.actionText||'реакция')+'». Реакция потрачена.':'Пропускает предложенную реакцию «'+String(request.actionText||'реакция')+'».',actionType:'reaction',actorKey:String(entry.key||''),targetKey:String(request.targetKey||''),reactionAccepted:accepted,manualResolution:true,ts:stamp};room.updatedAt=stamp;return room;
        }).then(function(result){if(!result||!result.committed){var messages={'room-not-found':'Комната больше недоступна.','combat-reaction-request-missing':'Запрос реакции уже закрыт.','combat-missing':'Сейчас нет активного боя.','combat-participant-missing':'Ваш герой не участвует в бою.','combat-status-blocked':'Текущее состояние запрещает реакцию.','combat-action-spent':'Реакция этого раунда уже потрачена.','combat-prepared-pending':'Мастер сначала должен завершить другое сработавшее действие.','combat-reaction-resolution-pending':'Мастер сначала должен завершить другую реакцию.'};throw roomError(messages[abortCode]||'Не удалось ответить на реакцию.',abortCode||'combat-reaction-request-missing');}return refreshRoom(session.code).then(function(){return api.getSnapshot();});});
      }).catch(function(error){if(error&&['player-only','master-only','room-not-found','combat-reaction-request-missing','combat-missing','combat-participant-missing','combat-status-blocked','combat-action-spent','combat-prepared-pending','combat-reaction-resolution-pending'].indexOf(error.code)>=0)throw error;throw friendlyFirebaseError(error);});
    },
