...
assert.match(network, /entry\.deadRoundOverride=combatParticipantIsDead\(entry\)\?!skipped:false/, 'returning a dead participant records an explicit GM override');
...
assert.match(html, /entry\.deadRoundOverride=combatEntryIsDead\(entry\)\?!entry\.skipRounds:false/, 'the Workshop GM toggle preserves the same dead-participant override');
