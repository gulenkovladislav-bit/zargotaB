...
assert.match(html, /inventoryAlreadyOpen=inventoryDrawer&&inventoryDrawer\.classList\.contains\('open'\)[^;]*activePanel==='inventory'[^;]*inventoryBody\.dataset\.activePanel==='inventory'/, 'the bag control closes only an already-open inventory panel, not a character sheet');
assert.match(html, /turningOn=!inventoryAlreadyOpen/, 'the bag control switches an open character sheet to inventory');
