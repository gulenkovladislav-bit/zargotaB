...
assert.match(html, /\.zg-vtt-party\.drawer-priority\{z-index:35\}/, 'the party strip must pass below the open backpack layer instead of covering its frame');
assert.match(html, /party=el\('zg-vtt-party'\);if\(party\)party\.classList\.add\('drawer-priority'\)/, 'opening the backpack applies the stable under-drawer party layer');
