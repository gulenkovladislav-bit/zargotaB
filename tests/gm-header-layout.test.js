...
assert.match(html, /\.zg-scene-tools-button\{[^}]*display:none!important;[^}]*min-width:116px\}/, 'overview and zones are hidden as one tool outside editing mode');
assert.match(html, /\.gm-edit-mode \.zg-scene-tools-button\{display:inline-flex!important;/, 'the combined scene tool appears only in editing mode');
