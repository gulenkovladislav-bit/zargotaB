...
assert.match(delivery, /function masterPanelDataSignature\(nextSnapshot\)/, 'the GM panel derives a narrow recipient-only snapshot signature');
assert.match(delivery, /var recipientDataChanged = nextSignature !== masterPanelSnapshotSignature;/, 'unrelated combat snapshots do not rebuild the delivery panel');
assert.match(delivery, /if \(recipientDataChanged && panel && panel\.classList\.contains\('open'\)\)/, 'the open panel refreshes only when recipient identity changes');
assert.match(delivery, /if \(deliveryPanelInteractionActive\(\)\) pendingMasterPanelRefresh = true;/, 'room snapshots must not replace a button or active delivery field');
assert.match(delivery, /panel\.addEventListener\('pointerdown'.*panelPointerActive = true/, 'a click remains protected from pointerdown through click dispatch');
assert.match(delivery, /setTimeout\(flushPendingMasterPanelRefresh, 0\)/, 'a necessary recipient refresh waits until the current click completes');
var deliverySignatureStart = delivery.indexOf('  function masterPanelDataSignature(nextSnapshot) {');
var deliverySignatureEnd = delivery.indexOf('\n  function currentForm()', deliverySignatureStart);
assert.ok(deliverySignatureStart >= 0 && deliverySignatureEnd > deliverySignatureStart, 'recipient signature helper must remain independently testable');
var signatureContext = {Object:Object,String:String,JSON:JSON,result:null};
vm.runInNewContext(delivery.slice(deliverySignatureStart, deliverySignatureEnd) + '\nresult = masterPanelDataSignature(input);', Object.assign(signatureContext, {input:{room:{code:'ROOM',members:{player:{uid:'player',role:'player',characterId:'hero',character:{id:'hero',name:'Герой',portrait:'hero.png',hpCur:12,statuses:['poisoned']}},gm:{uid:'gm',role:'master'}}}}}));
var firstRecipientSignature = signatureContext.result;
signatureContext.input.room.members.player.character.hpCur = 3;
signatureContext.input.room.members.player.character.statuses = ['stunned'];
signatureContext.input.room.combat = {active:true,round:4,turnIndex:2,updatedAt:999};
vm.runInNewContext('result = masterPanelDataSignature(input);', signatureContext);
assert.strictEqual(signatureContext.result, firstRecipientSignature, 'combat HP, statuses and turn ticks do not invalidate the delivery UI');
signatureContext.input.room.members.player.character.name = 'Герой после переименования';
vm.runInNewContext('result = masterPanelDataSignature(input);', signatureContext);
assert.notStrictEqual(signatureContext.result, firstRecipientSignature, 'a real recipient identity change still refreshes the delivery UI');
