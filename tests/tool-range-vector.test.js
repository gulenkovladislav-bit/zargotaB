...
assert.match(html, /function scheduleActiveToolPointerRefresh\(\)/, 'camera changes schedule one coalesced refresh for the last pointer position');
assert.match(html, /document\.elementFromPoint\(actionPointerX,actionPointerY\)/, 'camera refresh resolves the current token under the stationary viewport pointer');
assert.match(html, /world\.style\.transform[\s\S]*?scheduleActiveToolPointerRefresh\(\)/, 'zoom and pan realign the range path with the fixed viewport cursor');
assert.match(html, /toolRangeLayerRect=\{layer:layer,width:layer\.offsetWidth,height:layer\.offsetHeight\}/, 'the endpoint uses unscaled layer pixels so camera zoom is applied exactly once');
...
assert.match(html, /cursor\.classList\.toggle\('on-token',\(toolKey==='attack'\|\|toolKey==='ability'\)&&!!config\.targetHovered\)/, 'the fast cursor receives the same target-hover state as the range endpoint');
...
assert.match(html, /directThread=attackThread\|\|abilityThread\|\|movementThread/, 'movement, spell and ordinary attack targeting share the lightweight path mode');
assert.match(html, /allowedPath=directThread\?toolRangeStraightPath\(origin,geometry\.allowedPoint\)/, 'all high-frequency targeting bypasses decorative curve sampling on every pointer frame');
...
assert.match(html, /ornamentalThread=false/, 'ordinary attack targeting cannot restore the expensive sampled cubic curve');
...
assert.match(html, /\.zg-action-cursor\[data-tool="combat-attack"\]\.range-active \.zg-action-cursor-glyph\{opacity:1\}/, 'combat targeting also keeps the exact viewport cursor visible while the scene line is active');
assert.match(html, /\.zg-tool-range-vector\[data-tool="attack"\] \.range-vortex\{display:none\}/, 'the shifted scene-space attack endpoint cannot duplicate the exact viewport cursor');
assert.match(html, /\.zg-action-cursor\[data-tool="ability"\]\.on-token \.zg-action-cursor-glyph\{left:-36px;top:-36px;width:72px;height:72px\}/, 'spell targeting restores the approved expanded reticle over a valid token');
...
assert.match(html, /classList\.toggle\('zg-combat-attack-targeting',combatAttackTool\)/, 'ordinary attack targeting exposes its own temporary performance state');
assert.match(html, /html\.zg-combat-attack-targeting \.zg-vtt-status-vfx/, 'status animations pause while an ordinary attack is aimed');
assert.match(html, /combatAttackPreviewCache=attacker&&origin\?\{origin:origin,limit:combatClientRangeCells/, 'ordinary attack targeting caches attacker, origin, weapon range and label once');
assert.match(html, /var preview=combatAttackPreviewCache;\s*return preview&&preview\.origin/, 'pointer movement reuses the cached ordinary attack preview');
...
assert.match(html, /vector\.classList\.remove\('open','over-range','on-token','has-limit'\)/, 'range cleanup removes every visible and semantic vector state');
assert.match(html, /underlay\.classList\.remove\('open','over-range','has-limit'\)/, 'range cleanup removes every SVG underlay state');
assert.match(html, /rangeValue=vector\.querySelector\('\.range-distance-value'\);if\(rangeValue\)rangeValue\.textContent=''/, 'range cleanup removes the stale distance label');
assert.match(html, /underlay\.querySelectorAll\('path'\)[\s\S]*?path\.setAttribute\('d',''\)/, 'range cleanup removes every stale SVG path');
assert.match(html, /cursor\.classList\.remove\('over-range','range-active','on-token'\)/, 'range cleanup removes stale cursor feedback');
