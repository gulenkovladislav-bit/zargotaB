...
assert.match(html, /var combatAttackTargetKey='',combatAttackWeaponId=''/, 'player attack state must remember the selected weapon separately from the target');
assert.match(html, /w\.zgCombatAttackWeapon=function\(id\)/, 'player attack panel must allow choosing a weapon');
assert.match(html, /combatAttackTargetKey='';combatAttackError='Нельзя выбрать себя/, 'invalid map targets must clear a stale previous target');
