...
assert.match(html, /function combatPromptStatusModifiers\(/, 'the live prompt must resolve statuses inside the combat renderer scope');
assert.match(html, /function combatPromptStatusSource\(/, 'the live prompt must merge the presentation entry with Firebase combat, member and token status sources');
assert.match(html, /combatPromptStatusModifiers\(combatPromptStatusSource\(ownCombatEntry\)\)/, 'the attacker prompt must use the merged current status source');
assert.match(html, /promptRequest\.details&&promptRequest\.details\.mode\|\|promptRequest\.rollMode/, 'the approved attack prompt must restore the GM roll mode after reconnect');
assert.match(html, /promptHasDisadvantage=attackMode==='disadvantage'\|\|!!promptModifiers\.attackDisadvantage/, 'the approved attack prompt must display status-forced disadvantage before the roll');
assert.match(html, /Преимущество и помеха взаимно отменились/, 'the prompt must explain a canonical advantage/disadvantage cancellation in Russian');
assert.match(html, /Перевага і перешкода взаємно скасувалися/, 'the prompt must explain a canonical advantage/disadvantage cancellation in Ukrainian');
assert.match(network, /actionRequest\/rollMode'\] = acceptedCombatAttackMode/, 'the GM attack decision must persist a reconnect-safe top-level roll mode');
...
assert.match(network, /writeScene:!expiredSummons\.length&&\(masterAdvance\|\|String\(current\.uid\|\|''\)===String\(user\.uid\)\)/);
