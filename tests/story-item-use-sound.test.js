...
function run(item,action){let sounds=[],saved,errors=0;const ctx={c:{inventoryItems:[item]},item,index:0,mutate:rules.applyCreatureInventory,Date,t:(a)=>a,notice(){errors++;},save(c){saved=c;},showCharacter(){},onClose(){},Audio:function(path){sounds.push(path);this.play=()=>Promise.resolve();}};vm.createContext(ctx);vm.runInContext(handler+';act('+JSON.stringify(action)+');',ctx);return{sounds,saved,errors};}
