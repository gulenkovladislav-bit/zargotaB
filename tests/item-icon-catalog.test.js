...
assert.strictEqual(icons.resolveKey('sword'), 'sword', 'plain AI/catalog keys must resolve to media');
...
assert.doesNotMatch(html, /SHOP_EMOJI_POOL/, 'shop item picker must not retain a parallel emoji palette');
assert.doesNotMatch(html, /data-emoji=/, 'item creation UI must not expose emoji choices');
