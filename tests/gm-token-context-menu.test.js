...
assert.match(menuBlock, /openTokenCharacterSheet\(token\)/, 'hero sheet action opens the canonical bag instead of duplicating the GM panel');
assert.match(menuBlock, /Пульт ГМ/, 'single-token menu exposes the fast GM panel action');
assert.match(menuBlock, /zgGmInterventionOpenToken\(token\.id,'entity'\)/, 'GM panel action targets the token that opened the context menu');
