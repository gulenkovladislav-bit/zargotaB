...
assert.match(html, /spellTargetHover: function\(\) \{[\s\S]*?sampleSources\.targetHoverSpell[\s\S]*?group:'target-hover'/, 'ability hover uses a dedicated quiet recorded cue');
assert.match(html, /ZargotaSound\.spellTargetHover\)w\.ZargotaSound\.spellTargetHover\(\)/, 'ability targeting calls the spell-specific hover cue');
...
assert.match(html, /syncAbilityTargetHover\(null\);syncAttackTargetHover\(null\);clearToolRangeVector/, 'leaving the scene clears both targeting hover states');
