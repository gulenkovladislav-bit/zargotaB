...
var combatStartStart = html.indexOf('w.zgCombatStart=function');
var combatStartEnd = html.indexOf('var combatAdvanceBusy', combatStartStart);
var combatStartBlock = html.slice(combatStartStart, combatStartEnd);
assert.doesNotMatch(combatStartBlock, /publishScene\(/, 'initiative must not republish the whole scene and block on every image');
assert.doesNotMatch(combatStartBlock, /sceneToken:copyToken\(token,tokenIndex\)/, 'initiative must not serialize scene tokens again');
assert.match(combatStartBlock, /combatStartDeadline\(/, 'initiative transition must have a bounded Firebase wait');
assert.match(combatStartBlock, /try\{/,
  'initiative transition must surface synchronous preparation failures instead of leaving a dead button');
...
assert.match(network, /upsertSceneToken: function \(token\)/, 'network API must support adding one token to an already published scene');
var liveTokenStart = network.indexOf('upsertSceneToken: function (token)');
var liveTokenEnd = network.indexOf('\n\n    cueCamera:', liveTokenStart);
var liveTokenBlock = network.slice(liveTokenStart, liveTokenEnd);
assert.match(liveTokenBlock, /firebase\.update\(firebase\.ref\(db, 'rooms\/' \+ session\.code \+ '\/scene'\)/);
assert.match(liveTokenBlock, /tokens:tokens/);
assert.doesNotMatch(liveTokenBlock, /publishScene\(/, 'single-token publication must not publish unfinished scene edits');
assert.match(network, /removeSceneTokens: function \(tokenIds\)/, 'network API exposes a narrow cleanup for leaked Workshop tokens');
var removeTokenStart = network.indexOf('removeSceneTokens: function (tokenIds)');
var removeTokenEnd = network.indexOf('\n\n    cueCamera:', removeTokenStart);
var removeTokenBlock = network.slice(removeTokenStart, removeTokenEnd);
assert.match(removeTokenBlock, /scene\/tokens/);
assert.match(removeTokenBlock, /firebase\.runTransaction/);
assert.match(removeTokenBlock, /wanted\[String\(token\.id \|\| ''\)\]/);
assert.doesNotMatch(removeTokenBlock, /members|character|inventory|consum/, 'Workshop cleanup cannot touch player profiles, HP resources or inventory');
assert.doesNotMatch(removeTokenBlock, /publishScene\(/, 'Workshop cleanup never republishes the complete scene');
assert.match(html, /function scheduleLiveTokenPublish\(token\)/);
assert.match(html, /roomSnapshot\.room\.phase!==['"]playing['"]/);
assert.match(html, /markDirty\('Черновик · жетон добавлен'\);scheduleLiveTokenPublish\(token\)/);
var finishDragStart = html.indexOf('function finishTokenDrag');
var finishDragEnd = html.indexOf('function beginTokenDrag', finishDragStart);
var finishDragBlock = html.slice(finishDragStart, finishDragEnd);
assert.match(finishDragBlock, /scheduleLiveTokenPublish\(item\.t\)/, 'manual GM drag must publish the new token coordinates');
var movementStart = html.indexOf("var origin=actorToken?{x:actorToken.x,y:actorToken.y,tokenId:actorToken.id}:{};");
var movementEnd = html.indexOf(".catch(function(err)", movementStart);
var movementBlock = html.slice(movementStart, movementEnd);
assert.doesNotMatch(movementBlock, /animateLastMovement\(/, 'GM movement must animate once from the authoritative room snapshot');
