...
assert.match(combatStartBlock, /sceneToken:copyToken\(token,tokenIndex\)/, 'selected combat tokens travel with the initiative request');
...
assert.match(network, /startUpdates\['scene\/tokens'\]/, 'combat start atomically publishes the selected scene tokens');
