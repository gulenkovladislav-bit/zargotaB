...
assert.match(html, /\.zg-irl-pinned-card\.zg-irl-window-resized\{min-height:0\}/, 'a proportionally shrunken pinned card must not be held open by its unscaled min-content height');
...
assert.match(html, /var SCALE_KEYS=\{'scene-memo':true,'pin-npc':true,'pin-event':true\}/, 'scene memos and pinned cards must opt into uniform content scaling');
assert.match(html, /function ensureScaleContent\(node,grip\)[\s\S]*?zg-irl-scale-content[\s\S]*?layer\.appendChild\(child\)/, 'scalable notes must wrap all authored content in one transform layer');
assert.match(html, /function scaleContent\(node,width,height\)[\s\S]*?Math\.min\(\(Number\(width\)[\s\S]*?Number\(height\)[\s\S]*?--zg-irl-content-scale/, 'resizing must derive one uniform content scale from both window dimensions');
assert.match(html, /\.zg-irl-scale-content\{[^}]*width:calc\(100% \/ var\(--zg-irl-content-scale,1\)\)[^}]*transform:scale\(var\(--zg-irl-content-scale,1\)\)/, 'the shared scale layer must enlarge text, images and controls together without enlarging the outer frame twice');
...
assert.match(html, /function resizeMove\(event\)[\s\S]*?if\(isScalable\(resize\.node\)\)[\s\S]*?baseWidth\*candidateWidth\+baseHeight\*candidateHeight[\s\S]*?resize\.nextWidth=baseWidth\*factor;resize\.nextHeight=baseHeight\*factor/, 'scene memos and pinned cards must resize from one projected factor and preserve their natural aspect ratio');
assert.match(html, /stored\.w=Math\.round\(nextWidth\);stored\.h=Math\.round\(nextHeight\);[\s\S]*?stored\.baseW=[\s\S]*?stored\.baseH=[\s\S]*?save\(\)/, 'resized dimensions and their content-scale baseline must persist alongside window positions');
