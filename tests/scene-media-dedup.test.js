...
(async function(){
  ...
  var importStart=html.indexOf('  function putSceneImportRecord');
  var importEnd=html.indexOf('  function finishPreparedSceneImport',importStart);
  var importBlock=html.slice(importStart,importEnd);
  assert.match(importBlock,/saveLinkedSceneRecord\(record,record&&record\.updatedAt/);
  assert.match(importBlock,/hydrateSceneRecordMedia\(collision\)/);
  assert.ok(importBlock.indexOf('putSceneImportRecord(backupRecord)')<importBlock.indexOf('putSceneImportRecord(replacement)'));
  var restoreStart=html.indexOf('  w.zgSceneVersionRestore=function');
  var restoreEnd=html.indexOf('  function migrateLoadedHeroes',restoreStart);
  assert.match(html.slice(restoreStart,restoreEnd),/saveLinkedSceneRecord\(rec,stamp/);
  ...
  assert.match(html.slice(duplicateStart,duplicateEnd),/hydrateSceneRecordMedia\(r\)/);
  assert.match(html.slice(duplicateStart,duplicateEnd),/saveLinkedSceneRecord\(copy,stamp/);
