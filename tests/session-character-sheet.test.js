...
assert.match(html, /zg-journal3-paper-actions/, 'the GM journal preview groups edit and delete actions without covering the record');
assert.match(html, /gmDeleteJournalEntry/, 'the GM can remove a player journal record through the synchronized room API');
assert.match(html, /commitGmJournalCharacter/, 'the local Workshop applies the same GM journal edits without requiring Firebase');
