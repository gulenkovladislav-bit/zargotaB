(function(root,factory){
  ...
  function createEngine(options){
    ...
    function shape(contextValue,key,x,y,size,rotation,color){
      ...
      else if(key==='necro'){contextValue.beginPath();contextValue.arc(0,0,size,Math.PI*.25,Math.PI*1.75);contextValue.lineTo(0,0);contextValue.closePath();contextValue.fill();}
    ...
    function drawSpellPath(effect,elapsed,duration){
      var key=effect.command.preset.key,t=Math.max(0,Math.min(1,elapsed/duration)),colors=palette(key),start=effect.start,end=effect.end,dx=end.x-start.x,dy=end.y-start.y,segments=key==='lightning-spear'?9:3;
      context.save();context.globalAlpha=Math.sin(Math.PI*Math.min(1,t*1.35))*(key==='lightning-spear' ? .9 : .62);context.strokeStyle=colors[1];context.lineWidth=key==='lightning-spear'?3.5:2.5;context.shadowColor=colors[2];context.shadowBlur=key==='lightning-spear'?15:9;context.beginPath();context.moveTo(start.x,start.y);
      for(var segment=1;segment<=segments;segment++){var p=segment/segments,jitter=segment===segments?0:Math.sin(segment*7.13)*(key==='lightning-spear'?5:2);context.lineTo(start.x+dx*p-dy/(Math.sqrt(dx*dx+dy*dy)||1)*jitter,start.y+dy*p+dx/(Math.sqrt(dx*dx+dy*dy)||1)*jitter);}context.stroke();context.restore();
    }
    ...
    function renderFrame(timestamp){
      ...
      var remaining=[];effects.forEach(function(effect){var elapsed=Math.max(0,timestamp-effect.startedAt),duration=effect.command.durationMs||effect.command.preset.durationMs,key=effect.command.preset.key;if(elapsed>=duration){release(effect,'complete');return;}if(key==='projectile'||key==='fire-projectile')drawProjectile(effect,elapsed,duration,width,height);else if(key==='movement')drawMovement(effect,elapsed,duration);else if(key==='mist-teleport')drawMistTeleport(effect,elapsed,duration);else{if(key==='blood-transfer'||key==='lightning-spear')drawSpellPath(effect,elapsed,duration);drawBurst(effect,elapsed,duration);}remaining.push(effect);});effects=remaining;diagnostics.drawFrames+=1;if(runtime)runtime.noteFrame(now()-frameStarted);if(effects.length)frameId=requestFrame(renderFrame);else stopLoop();
    }
